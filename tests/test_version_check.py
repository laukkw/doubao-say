...
class VersionCheckTests(unittest.TestCase):
    def test_sources_are_aligned_at_first_release(self):
        self.assertEqual(VERSION_CHECK.validate(root=ROOT), "1.0.0")

    def test_exact_release_tag_is_accepted(self):
        self.assertEqual(VERSION_CHECK.validate("v1.0.0", ROOT), "1.0.0")

    def test_wrong_release_tag_is_rejected(self):
        with self.assertRaisesRegex(ValueError, "must be 'v1.0.0'"):
            VERSION_CHECK.validate("v1.0.1", ROOT)
