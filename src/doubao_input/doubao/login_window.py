...
class LoginWindow:
    ...
    def _setup(self) -> None:
        ...
        self._window.set_title(tr("Doubao Voice Input — Sign in", "豆包语音输入 — 登录"))
        ...
        def on_js_finish(source, result=None, *_args):
            ...
            try:
                ...
                if device_id and web_id:
                    ...
                    logger.info(
                        "Sign-in parameters extracted (%d cookies)",
                        len(cookies),
                    )
                    GLib.idle_add(callback, params)
                else:
                    logger.warning(
                        "Missing localStorage params: device_present=%s, web_present=%s",
                        bool(device_id),
                        bool(web_id),
                    )
                    GLib.idle_add(callback, None)
